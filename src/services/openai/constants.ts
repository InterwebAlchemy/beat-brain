export const OPEN_AI_BASE_URL = 'https://api.openai.com'

export const OPEN_AI_DEFAULT_TEMPERATURE = 0.75

export const OPEN_AI_RESPONSE_TOKENS = 2000
export const OPEN_AI_DEFAULT_MODEL_NAME = 'gpt-4' // 'gpt-3.5-turbo'

export const OPEN_AI_CHAT_COMPLETION_OBJECT_TYPE = 'chat.completion'

export const OPEN_AI_BOT_SETTINGS = {
  temperature: OPEN_AI_DEFAULT_TEMPERATURE,
  maxTokens: OPEN_AI_RESPONSE_TOKENS,
  modelName: OPEN_AI_DEFAULT_MODEL_NAME
}
